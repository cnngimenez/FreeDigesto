...
class LoginsController < ApplicationController
  ...
  skip_before_filter :requerir_login, :only => [:new, :create]
  ...
  def create
    ...
    login = Login.autenticar(params[:login][:nombre_usuario],params[:login][:clave])
  ...
  # DELETE /logins/1.xml
  #
  # Destruir un objeto "Login" equivale a decir que el usuario
  # se deslogea, sale del sistema.
  #
  def destroy
    @login = Login.find(params[:id])
    @login.destroy

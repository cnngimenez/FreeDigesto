...
class ApplicationController < ActionController::Base
  ...
  #Siempre que se quiera entrar a un a página, filtrar(ejecutar requerir_login()).
  before_filter :requerir_login
  ...
  def verificar_privilegios(accion)
      ...
      if (sessionid.nil?)
        ...
        return TienePrivilegio.usuario_tiene_privilegio(accion, session[:current_user_id])
      end
  ...
  def usuario_actual
    if (session[:current_user_id].nil?)
      ...
      @_usuario_actual ||= session[:current_user_id] && Usuario.find(session[:current_user_id])
    end
  end

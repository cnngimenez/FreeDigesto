...
class AgregarDescriptoresController < ApplicationController
  ...
  # +creando_norma+: Indica si se llamó desde el asistente (controller)
  #    "nueva_norma".
  ...
  # +norma+,+numero+: Número de la norma que se eligió(Usado por el
  #     formulario en index).
  def seleccionar_desc_gen
    if not verificar_privilegios(PoseeDescriptor::PuedeCrear)
      ...
      if not params[:id].nil?
        ...
        # no hay id, o sea, buscar por número de norma pasado por POST por el
        # index.html.erb
  ...
  #
  # Si el usuario agrega un descriptor particular A que pertenece al
  # descriptor general B se agrega A a la tabla posee_desc_part y B a la tabla
  # posee_descriptor indicando que la norma tiene tanto el descriptor
  # particular A como el descriptor general B.
  #
  # ¡NO SE PUEDE AGREGAR EL DESCRIPTOR PARTICULAR A SÓLO SIN AGREGAR EL B EN
  # LA TABLA posee_descriptor!
  # 
  #
  ...
  def agregar_desc_gen
    if not verificar_privilegios(PoseeDescriptor::PuedeCrear)
      ...
      if not error == ""
        flash[:notice] = "Hubo un error al agregar los siguientes " +
                         "descriptores: \n" + error
      else
      ...
      respond_to do |format|
        format.html {redirect_to :action => "seleccionar_desc_part",
                                 :id => params[:id],
  ...
  #   "nueva_norma".
  # +id+: _id_ de la norma que se eligió para agregar el descriptor.
  # +descriptorGen_id+: El _id_ del descriptor general para buscar los
  #   descriptores particulares que le corresponden.
    ...
    if not verificar_privilegios(PoseeDescriptor::PuedeCrear)
      ...
      if (not params[:descriptor_particular][:id].nil?) and
        ...
        if error == ""
          ...
          flash[:notice] = "Hubo problemas para agregar los siguientes " +
                           "descriptore particulares: "

...
class NuevaNormaController < ApplicationController
  ...
  def guardar_norma
    if not verificar_privilegios(Norma::PuedeCrear)
      ...
      #@norma.lugar_publicacion = LugarPublicacion.find(params[:crear_norma][:lugar_publicacion_id])
      ...
      if @norma.save
        ...
        respond_to do |format|
          format.html {redirect_to :action => "agregar_comentarios", :id => @norma.id}
          #format.xml {head :ok}
        end
      else
    ...
    if not verificar_privilegios(Norma::PuedeCrear)
      ...
      if @norma.save
        ...
        if params[:modificando].nil? or params[:modificando].to_s == "true"
          respond_to do |format|
            format.html {redirect_to :action => "agregar_citas", :id => @norma.id}
            format.xml {head :ok}
          end
        else
          #Vino de modificar_norma
          respond_to do |format|
            format.html {redirect_to :controller => "modificar_norma", :action => "modificar_norma", :id => @norma}
          end
        end
      else
  ...
  def agregar_citas
    if not verificar_privilegios(Norma::PuedeCrear)
      ...
      # La cita no es nil y la cita no es "" y además, no tiene privilegios para editar
  ...
  def guardar_texto
    if not verificar_privilegios(Norma::PuedeCrear)
      ...
      if @norma.save
        ...
        #respond_to do |format|
        #    format.html{render_component :controller => "agregar_descriptores", :action => "seleccionar_desc_gen"}
        #    #format.xml
        #end
        ...
        if params[:modificando].nil? or params[:modificando].to_s == "false"
          respond_to do |format|
              format.html {redirect_to :action => "planificar_vencimientos", :id => @norma}
          end
        else
          respond_to do |format|
            format.html {redirect_to :controller => "modificar_norma", :action => "modificar_norma", :id => @norma}
    ...
    if not verificar_privilegios(CambioEstado::PuedeCrear)
      ...
      respond_to do |format|
        if @cambio_estado.save
          ...
          flash[:notice] = "¡No se pudo crear la planificación de Cambio de Estado!"
          format.html { redirect_to :action => "planificar_vencimientos", :id => @norma }
     ...
     if not verificar_privilegios(RelacionaConNorma::PuedeCrear)
      ...
      respond_to do |format|
        if @relaciona_con_norma.save
          ...
          format.html {redirect_to :action => 'otra_relacion', :id => @norma_inicial}
        else
          flash[:notice] = '¡No se pudo crear la relación!'
          format.html { render :action => 'crear_relaciones', :id => @norma_inicial }

...
class BuscarPorFechaController < ApplicationController
  ...
  def resultados
    if not verificar_privilegios(Norma::PuedeConsultar)
      ...
      # Dependiendo de la selección (buscar por fecha de: sanción,
      # publicación ó promulgación) se arma la condición.
      #
      if (params[:buscar_fecha][:buscar_por] == 'sancion')
        ...
        str_fecha_desde = fecha_desde[0] + "-" + fecha_desde[1] + "-" +
                          fecha_desde[2]
        str_fecha_hasta = fecha_hasta[0] + "-" + fecha_hasta[1] + "-" +
                          fecha_hasta[2]
      ...
      # Si eligió buscar por todas, general y/o particular entonces no

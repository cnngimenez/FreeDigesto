...
class BuscarPorDescriptoresController < ApplicationController
  ...
  def buscar_normas
    if not verificar_privilegios(Norma::PuedeConsultar)
      ...
      # No hay información en buscar_desc... viene del formulario
      # anterior(otras_opciones) reacomodar la variable params para que pueda
      # ser aceptado por las condiciones que continúan y además porque los
      # formularios de esta view, generan esta misma estructura.
      ...
      @lstNormas = Norma.buscar_por_descriptor(descgen_id, descpart_id,
                                               otras_opciones)
  ...
  def vista_impresion
    if not verificar_privilegios(Norma::PuedeConsultar)
      ...
      # No hay información en buscar_desc... viene del formulario
      # anterior(otras_opciones) reacomodar la variable params para que
      # pueda ser aceptado por las condiciones que continúan y además porque
      # los formularios de esta view, generan esta misma estructura.
      if params[:buscar_desc].nil?
        params[:buscar_desc] = params
        params[:buscar_desc].update(params[:otras_opciones])
        params.delete(:otras_opciones)

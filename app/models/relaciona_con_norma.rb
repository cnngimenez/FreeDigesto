...
class RelacionaConNorma < ActiveRecord::Base
  #Privilegios:
  ...
  validates_presence_of :norma_a_id, :message => "¡Debe escoger una norma \"inicial\" existente!"
  validates_presence_of :norma_b_id, :message => "¡Debe escoger una norma \"final\" existente!"
  validates_uniqueness_of :norma_a_id, :scope => [:norma_b_id], :message => "¡Ya existe una relación con esas normas!"
  # Debe existir el tipo de relación que se está aplicando a ambas normas.
  validates_presence_of :tipo_relacion_id, :message => "¡Debe asignar un tipo de relación!"
  ...
  # Elimino de mi tabla (_relaciona_con_norma_) todas las tuplas que relacionen cualquier
  # norma con la norma dada por el id +norma_id+ que me es brindada por parámetro.
  #
  ...
  def RelacionaConNorma::eliminar_relaciones_por_norma(norma_id)
    RelacionaConNorma.delete_all(["norma_a_id = ? OR norma_b_id = ?", norma_id, norma_id])
  end
end

...
class PoseeDescPart < ApplicationRecord
  #Privilegios:
  ...
  validates_presence_of(:descriptor_particular ,
                        :message => "¡Debe escribir un descriptor válido!")
  ...
  validates_uniqueness_of(
    :norma_id,
    :scope => [:descriptor_particular_id],
    :message => "¡Ya existe esa norma relacionada con ese descriptor!")

  # Busco los posee_desc_parts correspondientes a esa norma cuyos
  # descriptores particulares pertenezcan a ese descriptor general.
  #
  # Devuelvo un Array que tienen posee_desc_parts.
  def PoseeDescPart::buscar_por_dg_norma(norma_id, desc_gen_id)
    # No olvidar que si no se incorpora en el select lo siguiente:
    # "SELECT posee_desc_parts"
    # puede devolver mal los datos, principalmente aquellos cuyos campos
    # sean de nombres idénticos.

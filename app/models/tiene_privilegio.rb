...
class TienePrivilegio < ActiveRecord::Base
  #Privilegios:
  ...
  validates_uniqueness_of :usuario_id, :scope => [:privilegio_id], :message => 'Ese usuario ya tiene ese privilegio.'

  #Devuelve true si el usuario con "usuario_id" tiene el privilegio para realizar
  #la acción "accion".
  ...
  def TienePrivilegio.usuario_tiene_privilegio(accion, usuario_id)
    ...
    #Si no se encontró(retornó nil) devolver false(no tiene privilegios porque no existe!)
    if (privid.nil?)
      return false
    end

    #Buscar con el usuario y con el privilegio.
    encontrado = TienePrivilegio.find_by_usuario_id(usuario_id, :conditions => ["privilegio_id = ?",privid])
